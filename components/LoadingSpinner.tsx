  ...
  return (
    <div className={`flex flex-col items-center justify-center p-4 ${className}`}>
      <div className="relative">
        <div className={`${sizeClasses[size]} border-2 border-slate-200 border-t-doc-red rounded-full animate-spin`}></div>
      </div>
      {message && (
        <p className="mt-3 text-slate-600 text-sm font-medium">{message}</p>
      )}
    </div>
  );
};
