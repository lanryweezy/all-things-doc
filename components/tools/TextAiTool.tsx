...
export const TextAiTool: React.FC<TextAiToolProps> = ({ toolId, onBack }) => {
  ...
  const renderConfigInput = () => {
    ...
    if (toolId === ToolID.CODE_MORPH) {
      return (
        <div className="mb-4">
           <label className="block text-sm font-medium text-doc-slate mb-2">Convert to Language</label>
           <select 
            value={configValue}
            onChange={(e) => setConfigValue(e.target.value)}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        <div className="space-y-6">
          <div>
            ...
            <textarea
              className={`w-full h-48 p-4 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-opacity-50 outline-none resize-none font-mono text-sm ${
                toolId === ToolID.CODE_MORPH ? 'focus:ring-amber-500' : 
                toolId === ToolID.UNIVERSAL_TRANSLATOR ? 'focus:ring-purple-500' : 
                'focus:ring-doc-red'
              }`}
              placeholder={toolId === ToolID.CODE_MORPH ? "console.log('Hello World');" : "Type or paste your text here..."}
              value={textInput}
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
             ...
             <Button 
               ...
               className={toolId === ToolID.CODE_MORPH ? "!bg-amber-600 hover:!bg-amber-700" : toolId === ToolID.UNIVERSAL_TRANSLATOR ? "!bg-purple-600 hover:!bg-purple-700" : ""}
               icon={<Sparkles size={18} />}
             >
               {toolId === ToolID.MAGIC_SUMMARIZER ? "Summarize" : 
                toolId === ToolID.UNIVERSAL_TRANSLATOR ? "Translate" : 
                toolId === ToolID.CODE_MORPH ? "Convert Code" : "Polish Text"}
             </Button>
          </div>
        </div>
        ...
        {result && (
          <ResultDisplay 
            title="Result" 
            content={result} 
            isCode={toolId === ToolID.CODE_MORPH}
          />
        )}
      </div>
    </div>
