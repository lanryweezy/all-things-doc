    ...
    <ToolLayout toolId={ToolID.HTML_TO_MARKDOWN} onBack={onBack} maxWidth="6xl">
      <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 min-h-[400px]">
        <div className="flex flex-col">
          <label className="block text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 transition-colors">
            Input HTML
          </label>
          <textarea
            value={input}
        ...
        <div className="flex flex-col">
          <label className="block text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 flex justify-between items-center transition-colors">
            ...
            {output && (
              <button
                onClick={handleCopy}
                className="text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 text-xs flex items-center font-semibold transition-colors"
              >
                {copied ? (
      ...
      <div className="mt-8 flex justify-center flex-shrink-0">
        <Button
          ...
          disabled={!input.trim()}
          className="bg-cyan-600 hover:bg-cyan-700 px-12 transition-colors"
