      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-8 transition-colors">
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest transition-colors">
              Base Dimension
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 ml-1 transition-colors">
                  Width
                </label>
                <input
                  type="number"
            ...
            <div className="bg-cyan-50 dark:bg-cyan-900/30 p-4 rounded-xl text-center transition-colors">
              <span className="text-xs font-bold text-cyan-400 uppercase mr-2 transition-colors">
                Calculated Ratio
              </span>
              <span className="text-2xl font-black text-cyan-600 dark:text-cyan-400 transition-colors">
                {ratio}
              </span>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group min-h-[200px] flex items-center justify-center shadow-2xl transition-all">
            ...
            />
            <div className="absolute bottom-6 right-8 text-[10px] font-black uppercase tracking-widest text-white/40">
              Visualizer (Scaled)
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm space-y-4 transition-colors">
            <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest px-1 transition-colors">
              Common Presets
            </h3>
            <div className="space-y-2">
              {presets.map(p => (
                ...
                >
                  <span>{p.name}</span>
                  <span className="font-mono text-[10px] font-black bg-slate-100 dark:bg-slate-950 px-2.5 py-1 rounded-lg transition-colors">
                    {p.w} x {p.h}
                  </span>
                </button>
              ))}
