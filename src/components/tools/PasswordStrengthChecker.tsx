      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-500 uppercase tracking-wider ml-1">Password to Check</label>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Requirements</h3>
            <div className="grid grid-cols-1 gap-2">
              {requirements.map((req, i) => {
                const isMet = req.regex.test(password);
                return (
                  <div key={i} className={`flex items-center space-x-3 p-3 rounded-2xl border transition-all ${isMet ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center">
            <div className="text-slate-400 font-bold uppercase tracking-widest text-xs mb-6">Strength Meter</div>
            ...
            <div className="relative w-48 h-48 flex items-center justify-center mb-6">
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center">
                 <div className={`text-3xl font-black ${strength.textColor}`}>{password ? Math.round(((strength.score + 1) / 8) * 100) : 0}%</div>
                 <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Secure</div>
               </div>
            </div>
          ...
          <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100 flex items-start space-x-4">
             ...
             <div className="text-xs text-amber-800 leading-relaxed font-medium">
               Passwords are never stored or transmitted. All checking logic runs entirely in your browser locally for maximum privacy.
             </div>
          </div>
        </div>
      </div>
